...
# TRAIN_GT_HU = open_file('/home/kamata/文档/DeepHyperX/DeepHyperX-master/Datasets/Houston/TRAIN_GT.mat')['TR']
# TEST_GT_HU = open_file('/home/kamata/文档/DeepHyperX/DeepHyperX-master/Datasets/Houston/TEST_GT.mat')['TE']
...
for run in range(N_RUNS):
    ...
    if TRAIN_GT is not None and TEST_GT is not None:
        ...
        # disjoint_train = convert_to_color(train_gt)
        # disjoint_test = convert_to_color(test_gt)
        # display_predictions(disjoint_train, viz, caption='disjoint TR samples')
        # display_predictions(disjoint_test, viz, caption='disjoint TE samples')
    ...
    display_predictions(convert_to_color(train_gt), viz, caption="Train ground truth") ## if you use 'random or fixed sampling'
    display_predictions(color_gt,viz,caption='Ground truth')
    display_predictions(convert_to_color(test_gt), viz, caption="Test ground truth") ## if you use 'random or fixed sampling'
    ...
    if MODEL == 'SVM_grid':
        ...
        probabilities, output_list = test(model, img, hyperparams)
        # output_list= torch.cat(output_list, dim=0)
        # print('output_size', output_list.shape, probabilities.shape)
        prediction1 = np.argmax(probabilities, axis=-1)
        prediction2 = np.argmax(probabilities, axis=-1)
        # print('predictions',prediction2.shape)
        # plot_tsne(output_list, prediction1)
